...
Viewer::Viewer() : nanogui::Screen(Eigen::Vector2i(1024, 768), "DGP Viewer") {
    ...
    new Label(window_, "Make the lamp:", "sans-bold");
    ...
    popupBtn = new PopupButton(window_, "Perforations");
    ...
    b = new Button(panel, "Start Perforating");
    b->setCallback([this]() {
        mesh_->digHole(sampligDistTextBox->value(),holeDiamTextBox->value());
        this->mesh_->compute_mesh_properties();
    ...
    new Label(panel, "Distance between samples:", "sans-bold");
    sampligDistTextBox = new FloatBox<float>(panel, 0.08);
    sampligDistTextBox->setEditable(true);
    sampligDistTextBox->setFixedSize(Vector2i(50, 20));
    sampligDistTextBox->setDefaultValue("0.08");
    sampligDistTextBox->setFontSize(16);
    sampligDistTextBox->setFormat("[-]?[0-9]*\\.?[0-9]+");
    new Label(panel, "Perforation radius:", "sans-bold");
    holeDiamTextBox = new FloatBox<float>(panel, 0.04);
    holeDiamTextBox->setEditable(true);
    holeDiamTextBox->setFixedSize(Vector2i(50, 20));
    holeDiamTextBox->setDefaultValue("0.04");
    holeDiamTextBox->setFontSize(16);
    holeDiamTextBox->setFormat("[-]?[0-9]*\\.?[0-9]+");


    popupBtn = new PopupButton(window_, "Solidifcation");
    popup = popupBtn->popup();
    popup->setLayout(new GroupLayout());
    ...
    b = new Button(panel, "Make solid");
    b->setCallback([this]() {
        mesh_->hullify(thicknessTextBox->value(),reductionTextBox->value());
        this->refresh_mesh();
    });
    ...
    layout->setSpacing(0, 10);
    panel->setLayout(layout);
    new Label(panel, "Hull thickess:", "sans-bold");
    thicknessTextBox = new FloatBox<float>(panel, 0.1);
    thicknessTextBox->setEditable(true);
    thicknessTextBox->setFixedSize(Vector2i(50, 20));
    thicknessTextBox->setDefaultValue("0.1");
    thicknessTextBox->setFontSize(16);
    thicknessTextBox->setFormat("[-]?[0-9]*\\.?[0-9]+");
    new Label(panel, "Mesh reduction (<=0 for no reduction):", "sans-bold");
    reductionTextBox = new FloatBox<float>(panel, -1);
    reductionTextBox->setEditable(true);
    reductionTextBox->setFixedSize(Vector2i(50, 20));
    reductionTextBox->setDefaultValue("-1");
    reductionTextBox->setFontSize(16);
    reductionTextBox->setFormat("[-]?[0-9]*\\.?[0-9]+");
